...
async function accept(id,b,spoken){if(busy||phase<0||phase>3)return;const expected=phase===3?['apple','blanket','umbrella'][recall]:levels[phase].id;
  if(id!==expected){b?.classList.remove('wrong');void b?.offsetWidth;b?.classList.add('wrong');say(levels[phase].say);demo();return;}
  ...
  setTimeout(()=>{if(g!==generation)return;phase++;busy=false;if(phase===4){scene.classList.remove('raining');$('#celebration').hidden=false;$('#thought').hidden=true;destination.classList.add('hidden');$('#mode').textContent=voiceCount?`语音参与 ${voiceCount} 次 · 不作能力评分`:'图片互动完成 · 尚未练习语音';}render();},250);
}
...
$('#mic').onclick=()=>{if(busy||phase<0||phase>3)return;if(recognition){stopListening();return;}const R=window.SpeechRecognition||window.webkitSpeechRecognition;if(!R){$('#mic').classList.add('unavailable');say('Watch me. You can use the pictures.');$('#mode').textContent='本浏览器没有语音识别 · 可用图片完成';demo();return;}window.picnicSpeech.stop();const r=new R();recognition=r;const g=generation,p=phase;r.lang='en-US';r.interimResults=false;r.maxAlternatives=1;
